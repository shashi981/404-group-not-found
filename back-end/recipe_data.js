...
async function insertDataIntoDatabase(jsonData) {       
    ...
    try{
        jsonData.meals.forEach(async (meal) => {        
            const recipeInfoQuery = `INSERT IGNORE INTO 
RECIPE_INFO (Rname, Category, RID, Instruction, YoutubeLink) VALUES (?, ?, ?, ?, ?);`
            ...
            for (let i = 1; i <= 20; i++) {
                ...
                if (meal[ingredientKey] !== '') {       
                    ...
                    const [results] = await con.promise().query(recipeQuery, [meal.idMeal, meal[ingredientKey], 
meal[measureKey]])
...
function saveDataToFile(jsonData, filename) {
    ...
    fs.writeFile(filePath, jsonStr, (err) => {
      if (err) {
        console.error('Error saving data to the file:', 
err)
      } else {
